---
permalink: /publication/
title: " "
author_profile: false
---

Publication
===========

Papers (1st author or with leading contributions):

 + [8] &nbsp;**W. Wang**, S. Cantalupo, M. Galbiati, et al. A&A, 711, A84 (2026, [arXiv: 2601.20473](https://arxiv.org/abs/2601.20473){:target="_blank"}): *A Quiescent Galaxy in a Gas-Rich Cosmic Web Node at z~3* ([joural highlight](https://www.aanda.org/component/content/article/214-highlights/2026-highlights/3209-a-quiescent-galaxy-in-a-gas-rich-cosmic-web-node-at-z-3){:target="_blank"})

 + [7] &nbsp;**W. Wang**, S. A. Kassin, S. M. Faber, D. C. Koo, T. M. Heckman et al. submitted to ApJ (2026, [arXiv: 2606.10116](https://arxiv.org/abs/2606.10116){:target="_blank"}): *The Importance of Galaxy-Wide Star Formation in Driving Winds at z~1*

 + [6] &nbsp;X. Wang, S. Cantalupo, **W. Wang**, et al. A&A, 710, A6 (2026, [arXiv: 2511.19608](https://arxiv.org/abs/2511.19608){:target="_blank"}): *Metal enrichment of galaxies in a massive node of the Cosmic Web at z~3*

 + [5] &nbsp;**W. Wang**, S. Cantalupo, A. Pensabene et al. Nature Astronomy 9, 710 (2025, [doi: 10.1038/s41550-025-02500-2](https://doi.org/10.1038/s41550-025-02500-2){:target="_blank"}): *A Giant Disk Galaxy Two Billion Years After The Big Bang* ([journal highlight](https://www.nature.com/articles/s41550-025-02574-y){:target="_blank"})

+ [4] &nbsp;A. Pensabene, S. Cantalupo, **W. Wang** et al. A&A 701, A120 (2025, [arXiv: 2507.16921](https://arxiv.org/abs/2507.16921){:target="_blank"}): \
*ALMA survey of a massive node of the Cosmic Web at z∼3: II. A dynamically cold disk galaxy in the proximity of a hyperluminous quasar*

 + [3] &nbsp;**W. Wang**, S. A. Kassin, S. M. Faber, D. C. Koo et al. ApJ 930, 146 (2022, [arXiv: 2109.12133](https://arxiv.org/abs/2109.12133){:target="_blank"}): *The Baltimore Oriole’s Nest: Cool Winds from the Inner and Outer Parts of a Star-Forming Galaxy at z = 1.3*

 + [2] &nbsp;**W. Wang**, S. A. Kassin, C. Pacifici et al. ApJ 869, 161 (2018, [arXiv:1811.03671](https://arxiv.org/abs/1811.03671){:target="_blank"}):\
  *Galaxy Inclination and the IRX-β Relation: Effects on UV Star Formation Rate Measurements at Intermediate to High Redshifts*

 + [1] &nbsp;**W. Wang**, S. M. Faber, F.-S. Liu et al. MNRAS 469, 4063 (2017, [arXiv:1705.05404](https://arxiv.org/abs/1705.05404){:target="_blank"}):\
  *UVI colour gradients of 0.4<z<1.4 star-forming main-sequence galaxies in CANDELS: dust extinction and star formation profiles*


There are 95 million Wang in the world. But running exact name matching (=author:'Wang, Weichen') and selecting exclusively the astronomy subfield so far work well to find me: 
 
[Up-to-date list on ADS](https://ui.adsabs.harvard.edu/search/filter_database_fq_database=NOT&filter_database_fq_database=*%3A*&filter_database_fq_database=database%3A%22physics%22&fq=%7B!type%3Daqp%20v%3D%24fq_database%7D&fq_database=(*%3A*%20NOT%20database%3A%22physics%22)&p_=0&q=(%3Dauthor%3A(%22Wang%2C%20Weichen%22)%20AND%20pubdate%3A%5B2016-01%20TO%20*%5D%20AND%20collection%3Aastronomy)%20OR%20(author%3A%22Wang%2C%20W%22%20AND%20author%3A%22Cantalupo%22%20AND%20collection%3Aastronomy)&sort=date%20desc%2C%20bibcode%20desc){:target="_blank"}

Curriculum Vitae
================
[PDF Version](http://WeichenStars.github.io/files/WWang_UNIMIB.pdf){:target="_blank"}